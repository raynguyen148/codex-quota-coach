...
export function render(result, options = {}) {
  ...
  if (options.compact) {
    if (quick) add(`[${quick.label}] Pace ${rate(quick.perDay, quick)} / budget ${rate(quick.safePerDay, quick)} (no manual reset; ${quick.confidence} confidence)`);
    ...
    add(`${offline ? 'CACHED · ' : ''}${windows.join(' | ')}${normalized?.ordinaryUsageAllowed === false ? ' | INCLUDED USAGE BLOCKED' : ''}${recommendation ? ` | ${recommendation.title}` : ''}`);
    ...
    if (['overview', 'forecast'].includes(command)) quickSection();
